...
class Intro(Scene):
    def construct(self):
        first_line = Text("CMPS446")
        first_line.set_color_by_gradient(BLUE, PURPLE)
        second_line = Text("Image Processing and Computer Vision")
...
The above is Course Intro, I create it by:
manim -qm project/intro.py Intro
...
class HoughTransformScene(Scene):
    def construct(self):
        ...
        self.wait(3)
```
And the above is HoughTransform first example, I run it by:
manim -qm project/hough.py HoughTransformScene


I want to combine both of them. We can have a screen in the middle Saying "Example One"